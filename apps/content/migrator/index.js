...
async function start () {
  ...
  if (migrationVersion === config.metadata.latestContentMigration) {
    console.log(
      "project migration version is the same as latest migration. exit migrator."
    );
    return;
  }
  ...
  await Configuration.updateOne(
    { key: MIGRATION_KEY, "metadata.isMigrationRunning": false },
    {
      $set: {
        metadata: {
  ...
  const {
    metadata: { isMigrationRunning, handler },
  } = await getCurrentMigrationHandler();
  ...
  if (migrationVersion > config.metadata.latestContentMigration) {
    return runMigrationsForward(config.metadata.latestContentMigration).then(
      clearCurrentMigrationHandler,
      clearCurrentMigrationHandler
    );
  }
